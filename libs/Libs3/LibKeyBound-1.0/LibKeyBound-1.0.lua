...
local MINOR = tonumber(string.match("$Revision: 98 $", "(%d+)")) + 90000
...
function LibKeyBound:Initialize()
	do
		...
		perChar:SetScript('OnShow', function()
			this:SetChecked(GetCurrentBindingSet() == 2)
		end)

		local current
		perChar:SetScript('OnClick', function()
		...
		okayBindings:SetScript('OnClick', function()
			current = (perChar:GetChecked() and 2) or 1
			SaveBindings(current)
			LibKeyBound:Deactivate()
		end)

		okayBindings:SetScript('OnHide', function()
			current = (perChar:GetChecked() and 2) or 1
			SaveBindings(current)
		end)

		okayBindings:SetScript('OnEvent', function()
			SaveBindings(current)
		...
		cancelBindings:SetScript('OnClick', function()
			LoadBindings(GetCurrentBindingSet())
			LibKeyBound:Deactivate()
		end)

		cancelBindings:SetScript('OnEvent', function()
			LoadBindings(GetCurrentBindingSet())
			this:UnregisterEvent(event)
			LibKeyBound:Deactivate()
		end)
...
function LibKeyBound:Set(button)
	...
	if button and self:IsShown() then
...
function LibKeyBound:ToShortKey(key)
	if key then
		...
		key = string.gsub(key, ' ', '')
		key = string.gsub(key, 'ALT%-', L['Alt'])
		key = string.gsub(key, 'CTRL%-', L['Ctrl'])
		key = string.gsub(key, 'SHIFT%-', L['Shift'])
		key = string.gsub(key, 'NUMPAD', L['NumPad'])
		...
		key = string.gsub(key, 'BACKSPACE', L['Backspace'])

		for i = 1, NUM_MOUSE_BUTTONS do
			key = string.gsub(key, 'BUTTON' .. i, L['Button' .. i])
		end

		key = string.gsub(key, 'CAPSLOCK', L['Capslock'])
		key = string.gsub(key, 'CLEAR', L['Clear'])
		key = string.gsub(key, 'DELETE', L['Delete'])
		key = string.gsub(key, 'END', L['End'])
		key = string.gsub(key, 'HOME', L['Home'])
		key = string.gsub(key, 'INSERT', L['Insert'])
		key = string.gsub(key, 'MOUSEWHEELDOWN', L['Mouse Wheel Down'])
		key = string.gsub(key, 'MOUSEWHEELUP', L['Mouse Wheel Up'])
		key = string.gsub(key, 'NUMLOCK', L['Num Lock'])
		key = string.gsub(key, 'PAGEDOWN', L['Page Down'])
		key = string.gsub(key, 'PAGEUP', L['Page Up'])
		key = string.gsub(key, 'SCROLLLOCK', L['Scroll Lock'])
		key = string.gsub(key, 'SPACEBAR', L['Spacebar'])
		key = string.gsub(key, 'SPACE', L['Spacebar'])
		key = string.gsub(key, 'TAB', L['Tab'])

		key = string.gsub(key, 'DOWNARROW', L['Down Arrow'])
		key = string.gsub(key, 'LEFTARROW', L['Left Arrow'])
		key = string.gsub(key, 'RIGHTARROW', L['Right Arrow'])
		key = string.gsub(key, 'UPARROW', L['Up Arrow'])
...
function LibKeyBound.Binder:Create()
	...
	binder:RegisterForClicks('LeftButtonUp', 'RightButtonUp', 'MiddleButtonUp', 'Button4Up', 'Button5Up')
...
function LibKeyBound.Binder:OnKeyDown(key)
	...
	if (key == 'UNKNOWN' or key == 'LSHIFT' or key == 'RSHIFT' or
		...
		key == 'SHIFT' or key == 'CTRL' or key == 'ALT') then
		return
	end
...
function LibKeyBound.Binder:OnMouseWheel()
	if arg1 > 0 then
		this:OnKeyDown('MOUSEWHEELUP')
	else
		this:OnKeyDown('MOUSEWHEELDOWN')
	end
end
...
function LibKeyBound.Binder:OnEnter()
	...
	if button then
		...
		local bindings = this:GetBindings(button)
		if bindings then
...
do
	...
	EventButton:SetScript('OnEvent', function()
		if (event == 'PLAYER_LOGIN' and not LibKeyBound.initialized) then
