local MAJOR, MINOR = 'UnitCasting-1.0', 1
...
--				Sea.io.print("tMod "..info.skill)
			...
			if uc.ChanneledHealsSpellcastsToTrack[info.skill] then
				newCast(info.victim, info.skill, true)
			end
	...
	--		local s = gsub(arg1, m, '%2')
		elseif info and info.type == "debuff" then
			if uc.InterruptsToTrack[info.skill] then
				forceHideTableItem(casts, info.victim)
				forceHideTableItem(heals, info.victim)
			end
			if uc.TimeModifierBuffsToTrack[info.skill] then
				Sea.io.print("tMod "..info.skill)
				newIBuff(info.victim, info.skill)
...
--[[		elseif frem then
			...
			if uc.TimeModifierBuffsToTrack[s] then
...
--			local t = gsub(arg1, m, '%2')
	...
	local channelDot = function(info)
		...
		if fchannelDotRes or fpchannelDotRes then
			...
			if uc.ChanneledSpellcastsToTrack[s] then
				newCast(c, s, true)
			end
		end
