...
function lib:Acquire(key, ...)
	...
	if select("#", unpack(arg)) > 0 then
		...
		local ok, msg = pcall(tooltip.SetColumnLayout, tooltip, unpack(arg))
...
function layoutCleaner:RegisterForCleanup(tooltip)
	layoutCleaner.registry[tooltip] = true
	layoutCleaner:Show()
end
...
function layoutCleaner:CleanupLayouts()
	...
	for tooltip in pairs(layoutCleaner.registry) do
		FixCellSizes(tooltip)
	end

	wipe(layoutCleaner.registry)
end
...
function tipPrototype:SetColumnLayout(numColumns, ...)
	...
	for i = 1, numColumns do
		local justification = select(i, unpack(arg)) or "LEFT"
...
function tipPrototype:AddColumn(justification)
	...
	local colNum = getn(self.columns) + 1
...
function ResetTooltipSize(tooltip)
	...
	SetTooltipSize(tooltip, max(0, (h_margin * (getn(tooltip.columns) - 1)) + (h_margin / 2)), 2)
end
...
function FixCellSizes(tooltip)
	...
	while next(colspans) do
		...
		for colRange, width in pairs(colspans) do
			local left, right = match(colRange, "^(%d+)%-(%d+)$")
		...
		if maxNeedCols then
			local left, right = match(maxNeedCols, "^(%d+)%-(%d+)$")
...
local function _SetCell(tooltip, lineNum, colNum, value, font, justification, colSpan, provider, ...)
	...
	local tooltipWidth = getn(tooltip.columns)
	local rightColNum
	...
	local width, height = cell:SetupCell(tooltip, value, justification, font, unpack(arg))
	cell:Show()
...
do
	local function CreateLine(tooltip, font, ...)
		if getn(tooltip.columns) == 0 then
			error("column layout should be defined before adding line", 3)
		end

		local lineNum = getn(tooltip.lines) + 1
		...
		for i = 1, getn(tooltip.columns) do
			local value = select(i, unpack(arg))
	...
	function tipPrototype:AddLine(...)
		return CreateLine(self, self.regularFont, unpack(arg))
	end

	function tipPrototype:AddHeader(...)
		local line, col = CreateLine(self, self.headerFont, unpack(arg))
...
function tipPrototype:SetColumnTextColor(colNum, r, g, b, a)
	...
	for lineIndex = 1, getn(self.lines) do
...
do
	...
	function tipPrototype:SetCell(lineNum, colNum, value, ...)
		...
		elseif colNum < 1 or colNum > getn(self.columns) then
			error("column number out of range: " .. tostring(colNum), 2)
		end

		-- Variable _argument checking
		local font, justification, colSpan, provider
		local i, _arg = 1, unpack(arg)

		if _arg == nil or checkFont(_arg, 2, true) then
			i, font, _arg = 2, unpack(arg)
		end

		if _arg == nil or checkJustification(_arg, 2, true) then
			i, justification, _arg = i + 1, select(i, unpack(arg))
		end

		if _arg == nil or type(_arg) == "number" then
			i, colSpan, _arg = i + 1, select(i, unpack(arg))
		end

		if _arg == nil or type(_arg) == "table" and type(_arg.AcquireCell) == "function" then
			i, provider = i + 1, _arg
		end

		return _SetCell(self, lineNum, colNum, value, font, justification, colSpan, provider, select(i, unpack(arg)))
	end
end -- do-block
...
function tipPrototype:GetLineCount()
	return getn(self.lines)
end

function tipPrototype:GetColumnCount()
	return getn(self.columns)
end
...
local scripts = {
	OnEnter = function(...)
		highlightFrame:SetParent(this)
		highlightFrame:SetAllPoints(this)
		highlightFrame:Show()

		if this._OnEnter_func then
			this:_OnEnter_func(this._OnEnter_arg, unpack(arg))
		end
	end,
	OnLeave = function(...)
		highlightFrame:Hide()
		...
		if this._OnLeave_func then
			this:_OnLeave_func(this._OnLeave_arg, unpack(arg))
		end
	end,
	OnMouseDown = function(...)
		this:_OnMouseDown_func(this._OnMouseDown_arg, unpack(arg))
	end,
	OnMouseUp = function(...)
		this:_OnMouseUp_func(this._OnMouseUp_arg, unpack(arg))
	end,
	OnReceiveDrag = function(...)
		this:_OnReceiveDrag_func(this._OnReceiveDrag_arg, unpack(arg))
	end
}
...
local function AutoHideTimerFrame_OnUpdate()
	this.checkElapsed = this.checkElapsed + arg1

	if this.checkElapsed > 0.1 then
		if MouseIsOver(this.parent) or (this.alternateFrame and MouseIsOver(this.alternateFrame)) then
			this.elapsed = 0
		else
			this.elapsed = this.elapsed + this.checkElapsed

			if this.elapsed >= this.delay then
				lib:Release(this.parent)
			end
		end
...
local function PrintStats()
	local tipCache = tostring(getn(tooltipHeap))
	local frameCache = tostring(getn(frameHeap))
	local tableCache = tostring(getn(tableHeap))
	local header = false
