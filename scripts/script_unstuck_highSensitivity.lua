script_unstuck_highSensitivity = {
	timer = 0,
	message = 'Unstuck...',
	pause = false,
	_lx = 0,
...
function script_unstuck_highSensitivity:DeBugInfo()
	-- color
	local r = 255;
	local g = 2;
	local b = 233;
...
function script_unstuck_highSensitivity:draw()

	script_unstuck_highSensitivity:DeBugInfo();

	script_unstuck_highSensitivity:drawChecks();
end

function script_unstuck_highSensitivity:drawChecks()
...
function script_unstuck_highSensitivity:turn(changeAngle)
...
function script_unstuck_highSensitivity:walkForward(yards)
	_lx, _ly, _lz = GetLocalPlayer():GetPosition();
...
function script_unstuck_highSensitivity:getSlope(yardsInfront)
...
function script_unstuck_highSensitivity:jumpObstacles()

	if ( (script_unstuck_highSensitivity:getObsMin(1) >= 0.3 and script_unstuck_highSensitivity:getObsMax(1) < 2.2 and script_unstuck_highSensitivity:getObsMax(1) > 1) or 
		(script_unstuck_highSensitivity:getObsMin(2) >= 0.3 and  script_unstuck_highSensitivity:getObsMax(2) < 2.2 and script_unstuck_highSensitivity:getObsMax(2) > 1) ) then
		--self.message = "Jumping over obstacle";
		JumpOrAscendStart();
	end
end

function script_unstuck_highSensitivity:unstuck()
	if (script_unstuck_highSensitivity:pathClearAuto(1) or script_unstuck_highSensitivity:pathClearAuto(2)) then
		script_unstuck_highSensitivity:walkForward(2);
	end
end

function script_unstuck_highSensitivity:pathClearAuto(yardsInfront)

	-- Jump over obstacles
	if (IsMoving()) then script_unstuck_highSensitivity:jumpObstacles(); end
	...
	for i = 1, 2 do	
		...
		for y = 1, yardsInfront do
			...
			if(not hitM and not hitL) then
				...
				--DEFAULT_CHAT_FRAME:AddMessage('script_unstuck_highSensitivity: Turning left.');
				script_unstuck_highSensitivity:turn(3.14/2);
				return false;
			end
			...
			if(not hitM and not hitR) then
				...
				--DEFAULT_CHAT_FRAME:AddMessage('script_unstuck_highSensitivity: Turning right.');
				script_unstuck_highSensitivity:turn(-3.14/2);
				return false;
			end
			...
			if(not hitL) then
				...
				self.message = "Path not clear, turning left..."
				--DEFAULT_CHAT_FRAME:AddMessage('script_unstuck_highSensitivity: Turning left.');
				script_unstuck_highSensitivity:turn(0.5);
				return false;
			end
...
function script_unstuck_highSensitivity:getObsMin(yardsInfront)
...
function script_unstuck_highSensitivity:getObsMax(yardsInfront)
...
function script_unstuck_highSensitivity:run()
	...
	if (not IsMoving()) then
		...
		self.message = "Slope Z: " .. script_unstuck_highSensitivity:getSlope(3) .. ' | Obstacle min-Z: ' .. script_unstuck_highSensitivity:getObsMin(2) .. ' | Obstacle max-Z: ' .. script_unstuck_highSensitivity:getObsMax(2);
	end

	if (script_unstuck_highSensitivity:pathClearAuto(2)) then
	else
		StopMoving();
	end
end
