		...
		-- Combat WE ARE NOW IN COMBAT
			...
			if (localObj:HasDebuff('Rabies') or localObj:HasDebuff('Corrosive Poison') or localObj:HasDebuff('Poison') or localObj:HasDebuff('Fevered Fatigue')
				or localObj:HasDebuff('Dark Sludge') or localObj:HasDebuff('Corrosive Poison') or localObj:HasDebuff('Slowing Poison')
					or localObj:HasDebuff('Infected Bite')) then
			...
			if (targetObj:GetDistance() < self.meeleDistance) then
				...
				-- Rester timer reset
				if (self.enableResterTime) then
					self.resterTime = true;
				end
			end
			return 0;
		end
...
function script_paladin:rest()
	...
	--Wait timer after combat
	if (self.enableResterTime) then
		if (self.resterTime) then
			if (localHealth > 1) and (not IsInCombat()) then
				self.waitTime = GetTimeEX() + (self.resterTimer * 1000);
				self.message = "Rest Timer Enabled resting for " .. self.resterTimer .. " seconds";
				if (self.resterTime) then
					self.resterTime = false;
				end
				return true;
			end
		end
...
function script_paladin:menu()
	if (CollapsingHeader("Paladin - Retribution")) then
		...
		wasClicked, self.enableResterTime = Checkbox("Rest after Combat On/Off (seconds)", self.enableResterTime);
		Text('If there is no need drink/eat we can rest to look human');
		self.resterTimer = SliderInt("Rest after combat", 1, 10, self.resterTimer);
		Separator();
		Text('Combat options:');
		Separator();
