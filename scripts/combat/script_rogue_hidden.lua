...
function script_rogue:menu()
	if (CollapsingHeader("[Hidden - Rogue Combat Options")) then
		...
		self.cpGeneratorCost = SliderFloat("Energy", 20, 50, self.cpGeneratorCost);
		...
		self.adrenRushComboHP = SliderFloat("Self Health below percent", 15, 75, self.adrenRushComboHP);
		Separator();
	end
end